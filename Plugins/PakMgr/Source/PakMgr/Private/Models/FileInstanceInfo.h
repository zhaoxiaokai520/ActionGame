...
class FFileInstanceInfo
	: public TSharedFromThis<FFileInstanceInfo>
	, public IFileInstanceInfo
{
public:
	 ...
	 */
	FFileInstanceInfo(const FGuid& InInstanceId, const TSharedRef<IFileInfo>& InOwner, const TSharedRef<IMessageBus, ESPMode::ThreadSafe>& InMessageBus);
...
public:
	 ...
	 * @param Context The message context.
	 */
	//void UpdateFromMessage(const FFileServicePong& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context);
...
public:	
	...
	virtual const TArray<TSharedPtr<FFileItemInfo>>& GetLog() override
	{
		return FileItemInfo;
	}

	virtual TSharedPtr<IFileInfo> GetOwnerSession() override
	{
		return Owner.Pin();
	}
	...
	DECLARE_DERIVED_EVENT(FFileInstanceInfo, IFileInstanceInfo::FLogReceivedEvent, FLogReceivedEvent);
...
private:
	...
	//void HandleFileLogMessage(const FFileServiceLog& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context);
